...
public class LearningCurve : MonoBehaviour
{
    ...
    // 1. 배열
    // 배열은 모든 타입의 값을 저장할 수 있으며, 모든 요소는 동일한 타입이어야 한다.
    // 배열이 생성될 때 배열의 길이나 요소의 갯수가 설정되며, 추후에 변경이 불가능하다.
    // 배열이 생성될 때 초기값이 지정되지 않으면, 각 요소에는 기본값이 지정된다. 숫자 타입을 저장하는 배열은 0으로 설정되고, 다른 타입은 null 로 설정되거나 미설정 상태가 된다.
    // 배열은 컬렉션타입중 유연성이 가장 떨어진다. 배열이 생성되고 나면 요소의 추가나 삭제가 불가능 하기 때문이다.
    
    // 2. 리스트
    // 리스트는 배열과 밀접한 관련이 있으며 단일 변수에서 같은 타입의 다양한 값을 수집한다.
    // 리스트는 요소의 추가, 삭제, 업데이트를 처리하기가 훨씬 쉬우며, 요소가 순차적으로 저장된다. 또한 리스트는 변경이 용이하다.
    // 리스트는 항목의 길이나 수를 변경할 수 있지만 배열보다 성능상의 비용이 많이 들기도 한다.
